    ...
    <html lang="en">
      ...
      <body className="min-h-screen bg-tank-black grid-pattern m-0 p-0">
        <Script id="native-shell-boot" strategy="beforeInteractive">
          {`(function(){function d(){var c=window.Capacitor;if(c&&c.isNativePlatform&&c.isNativePlatform())return true;if(window.matchMedia('(display-mode: standalone)').matches)return false;if(window.navigator.standalone)return false;var u=navigator.userAgent;if(!/iPhone|iPod|iPad/.test(u))return false;return/AppleWebKit/.test(u)&&/Mobile\\//.test(u)&&!/Safari\\//.test(u)}function p(){var c=window.Capacitor;if(c&&c.getPlatform)return c.getPlatform();if(/Android/.test(navigator.userAgent))return'android';if(/iPhone|iPod|iPad/.test(navigator.userAgent))return'ios';return'unknown'}function s(){if(!d())return;var r=document.documentElement,b=document.body;if(!b)return;r.classList.add('capacitor-native');b.classList.add('capacitor-native-body');var pl=p();if(pl&&pl!=='unknown')r.classList.add(pl);var e=document.createElement('div');e.style.cssText='position:fixed;top:0;padding-top:env(safe-area-inset-top);padding-top:constant(safe-area-inset-top);visibility:hidden;pointer-events:none';b.appendChild(e);var t=parseFloat(getComputedStyle(e).paddingTop)||0;e.style.cssText='position:fixed;bottom:0;padding-bottom:env(safe-area-inset-bottom);padding-bottom:constant(safe-area-inset-bottom);visibility:hidden;pointer-events:none';var bt=parseFloat(getComputedStyle(e).paddingBottom)||0;e.remove();if(pl==='ios'){if(t<1)t=47;if(bt<1)bt=34}else if(pl==='android'){var vv=window.visualViewport;if(vv){if(t<1&&vv.offsetTop>0)t=Math.round(vv.offsetTop);if(bt<1){var ib=window.innerHeight-vv.height-vv.offsetTop;if(ib>0)bt=Math.round(ib)}}if(t<1)t=28;if(bt<1)bt=48}r.style.setProperty('--app-safe-top',t+'px');r.style.setProperty('--app-safe-bottom',bt+'px');window.__AIM_NATIVE_SHELL__=true}s();document.addEventListener('DOMContentLoaded',s);setTimeout(s,0);setTimeout(s,100)})();`}
        </Script>
