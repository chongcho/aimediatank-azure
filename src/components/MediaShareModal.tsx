...
import { buildLinkedInShareUrl, buildXShareUrl } from '@/lib/socialLinkShare'
...
export default function MediaShareModal({
  ...
  const warmSharePreviewCaches = useCallback(() => {
    warmSocialSharePreview({
      mediaId,
      cardImageUrl: kakaoShareImageUrl,
      thumbnailUrl,
    })
  }, [mediaId, kakaoShareImageUrl, thumbnailUrl])
  ...
  useEffect(() => {
    ...
    warmSharePreviewCaches()
  }, [open, mediaId, warmSharePreviewCaches])
      ...
      <div className={`${APP_MODAL_SHELL_CLASS} max-w-md max-h-[min(90vh,720px)] flex flex-col`}>
        ...
        <div className="flex items-center justify-center gap-4 flex-wrap">
          ...
          {shareAppsEnabled.x !== false && (
            <button
              type="button"
              onClick={() => openSocialShareLink(buildXShareUrl(shareUrl, shareTitle))}
              className="flex flex-col items-center gap-1 text-gray-400 hover:text-white transition-colors"
              title="X"
            >
              <svg className="w-8 h-8" viewBox="0 0 24 24" fill="currentColor">
          ...
          {shareAppsEnabled.linkedin !== false && (
            <button
              type="button"
              onClick={() => openSocialShareLink(buildLinkedInShareUrl(shareUrl))}
              className="flex flex-col items-center gap-1 text-gray-400 hover:text-white transition-colors"
              title="LinkedIn"
            >
              <svg className="w-8 h-8" viewBox="0 0 24 24" fill="currentColor">
