...
function TalkChatContent({ onClose }: { onClose: () => void }) {
  ...
  const voiceCallSearching = voiceCallSearchQuery.trim().length > 0

  const formatVoiceCallStatus = (status: string) => {
    if (status === 'missed') return tr[TC.voiceCallStatusMissed]
    if (status === 'rejected') return tr[TC.voiceCallStatusRejected]
    return tr[TC.voiceCallStatusEnded]
  }

  const formatVoiceCallRecordSubtitle = (record: {
    direction: 'incoming' | 'outgoing'
    status: string
    at: string
  }) => {
    const direction =
      record.direction === 'incoming' ? tr[TC.voiceCallIncoming] : tr[TC.voiceCallOutgoing]
  ...
  const measureComposerMaxHeight = useCallback(() => {
    ...
    const formTotal = formEl.offsetHeight
    const textareaCurrent = textareaEl.offsetHeight
    const formOverhead = Math.max(0, formTotal - textareaCurrent)
    const available = messagesEl.clientHeight + formTotal
        ...
        >
          ...
          {showMentionPicker && mentionUsers.length > 0 && (
            <div 
              ...
              style={{
                position: 'absolute',
                bottom: `${(composerFormHeightPx || composerHeightPx) + MESSAGE_COMPOSER_PICKER_GAP_PX}px`,
                left: '12px',
                width: '220px',
                background: 'white',
                borderRadius: '8px',
