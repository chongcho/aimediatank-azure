# AiMediaTank Policy Book

**Effective Date:** December 20, 2024  
**Last Updated:** December 20, 2024

---

## Table of Contents

1. [Terms of Service](#1-terms-of-service)
2. [Privacy Policy](#2-privacy-policy)
3. [Content Guidelines](#3-content-guidelines)
4. [AI-Generated Content Policy](#4-ai-generated-content-policy)
5. [Copyright and Intellectual Property](#5-copyright-and-intellectual-property)
6. [User Conduct](#6-user-conduct)
7. [Payment and Refund Policy](#7-payment-and-refund-policy)
8. [Account Terms](#8-account-terms)
9. [Data Retention Policy](#9-data-retention-policy)
10. [Contact Information](#10-contact-information)

---

## 1. Terms of Service

### 1.1 Acceptance of Terms

By accessing or using AiMediaTank ("the Platform," "we," "us," or "our"), you agree to be bound by these Terms of Service. If you do not agree to these terms, please do not use our services.

### 1.2 Eligibility

- You must be at least 18 years old to use this Platform.
- You must have the legal capacity to enter into a binding agreement.
- You must not be prohibited from using the Platform under applicable laws.

### 1.3 Description of Service

AiMediaTank is a community platform for sharing, discovering, and purchasing AI-generated media content including:
- Videos
- Images
- Music

### 1.4 Modifications to Terms

We reserve the right to modify these terms at any time. Users will be notified of significant changes via email or platform notification. Continued use of the Platform after changes constitutes acceptance of the new terms.

---

## 2. Privacy Policy

### 2.1 Information We Collect

**Personal Information:**
- Name and username
- Email address
- Profile information (avatar, bio)
- Payment information (processed securely through Stripe)

**Usage Information:**
- Content views and interactions
- Upload history
- Purchase history
- Device and browser information
- IP address

### 2.2 How We Use Your Information

- To provide and maintain our services
- To process transactions and payments
- To communicate with you about your account
- To send service-related notifications
- To improve our Platform and user experience
- To detect and prevent fraud or abuse

### 2.3 Information Sharing

We do not sell your personal information. We may share information with:
- Payment processors (Stripe) for transaction processing
- Cloud service providers (Microsoft Azure) for hosting
- Law enforcement when required by law

### 2.4 Cookies and Tracking

We use cookies and similar technologies to:
- Maintain your session
- Remember your preferences
- Analyze Platform usage

### 2.5 Data Security

We implement industry-standard security measures including:
- Encrypted data transmission (HTTPS/TLS)
- Secure password hashing
- Regular security audits

### 2.6 Your Rights

You have the right to:
- Access your personal data
- Correct inaccurate data
- Delete your account and associated data
- Export your data
- Opt-out of marketing communications

---

## 3. Content Guidelines

### 3.1 Acceptable Content

Content uploaded to AiMediaTank must be:
- AI-generated or AI-assisted
- Original or properly licensed
- Appropriate for a general audience (unless marked otherwise)
- Compliant with all applicable laws

### 3.2 Prohibited Content

The following content is strictly prohibited:
- Child sexual abuse material (CSAM)
- Non-consensual intimate imagery
- Content promoting violence or terrorism
- Hate speech targeting protected groups
- Content infringing on intellectual property rights
- Malware or malicious code
- Spam or misleading content
- Content depicting illegal activities
- Deepfakes of real individuals without consent

### 3.3 Content Moderation

- We reserve the right to remove any content that violates these guidelines
- Repeated violations may result in account suspension or termination
- Users can report content violations through our support system

### 3.4 Age-Restricted Content

- Content intended for mature audiences must be appropriately labeled
- Users must verify their age to access age-restricted content

---

## 4. AI-Generated Content Policy

### 4.1 Definition

AI-generated content refers to media created using artificial intelligence tools including but not limited to:
- Image generators (Midjourney, DALL-E, Stable Diffusion, etc.)
- Video generators (Runway, Pika, Sora, etc.)
- Music generators (Suno, Udio, etc.)

### 4.2 Disclosure Requirements

- All content must be identified as AI-generated
- Users should specify the AI tool used when uploading content
- Misrepresenting AI content as human-created is prohibited

### 4.3 AI Tool Compliance

Users are responsible for:
- Complying with the terms of service of the AI tools they use
- Ensuring they have rights to commercialize AI-generated content
- Understanding licensing restrictions of their AI tools

### 4.4 Training Data Concerns

- Do not upload content that may have been generated using unauthorized training data
- Report content that appears to infringe on artists' or creators' rights

---

## 5. Copyright and Intellectual Property

### 5.1 Ownership

- You retain ownership of content you create and upload
- By uploading, you grant AiMediaTank a non-exclusive license to display, distribute, and promote your content on the Platform

### 5.2 Copyright Infringement

We respect intellectual property rights and comply with the Digital Millennium Copyright Act (DMCA).

**To file a DMCA takedown notice, provide:**
- Your contact information
- Identification of the copyrighted work
- Identification of the infringing material
- A statement of good faith belief
- A statement of accuracy under penalty of perjury
- Your physical or electronic signature

Send DMCA notices to: support@aimediatank.com

### 5.3 Counter-Notification

If your content was removed and you believe it was done in error, you may submit a counter-notification with:
- Your contact information
- Identification of the removed material
- A statement under penalty of perjury
- Consent to jurisdiction
- Your physical or electronic signature

### 5.4 Repeat Infringers

Accounts with repeated copyright violations will be terminated.

---

## 6. User Conduct

### 6.1 Expected Behavior

Users are expected to:
- Treat other users with respect
- Provide accurate information
- Protect their account credentials
- Report violations and suspicious activity

### 6.2 Prohibited Activities

Users must not:
- Harass, bully, or threaten other users
- Create multiple accounts to evade bans
- Attempt to hack or exploit the Platform
- Scrape or collect user data without permission
- Manipulate ratings or reviews
- Engage in fraudulent transactions
- Impersonate other users or entities

### 6.3 Consequences

Violations may result in:
- Warning
- Temporary suspension
- Permanent account termination
- Legal action where appropriate

---

## 7. Payment and Refund Policy

### 7.1 Pricing

- Content creators set their own prices
- Membership plans are priced as displayed on the Platform
- All prices are in US Dollars (USD)

### 7.2 Payment Processing

- Payments are processed securely through Stripe
- We accept major credit cards and other payment methods supported by Stripe
- Payment information is never stored on our servers

### 7.3 Membership Plans

| Plan | Price | Features |
|------|-------|----------|
| Free | $0/month | View content, limited uploads |
| Creator | $1.99/month | Upload content, sell creations |
| Premium | $9.99/month | Unlimited uploads, priority support |

### 7.4 Upload Fees

- Free tier: Limited free uploads per month
- Paid uploads: $1.00 per upload after free tier exhausted
- Premium members: Unlimited free uploads

### 7.5 Refund Policy

**Digital Content Purchases:**
- Due to the nature of digital content, all sales are final
- Refunds may be issued for technical issues preventing access
- Refund requests must be submitted within 7 days of purchase

**Membership Subscriptions:**
- Cancel anytime before the next billing cycle
- No refunds for partial billing periods
- Access continues until the end of the paid period

### 7.6 Creator Payouts

- Creators receive 70% of content sales
- Minimum payout threshold: $10
- Payouts processed monthly via Stripe Connect

---

## 8. Account Terms

### 8.1 Account Creation

- One account per person
- Accurate information required
- You are responsible for maintaining account security
- Notify us immediately of unauthorized access

### 8.2 Account Verification

- Email verification required for all accounts
- Additional verification may be required for sellers

### 8.3 Account Termination

**By User:**
- You may delete your account at any time
- Some data may be retained as required by law

**By AiMediaTank:**
- We may suspend or terminate accounts for:
  - Terms of Service violations
  - Illegal activity
  - Extended inactivity (24+ months)
  - Fraudulent activity

### 8.4 Data After Termination

- Uploaded content will be removed
- Transaction records retained for legal/tax purposes
- Personal data deleted within 30 days (unless legally required)

---

## 9. Data Retention Policy

### 9.1 Content Retention

- Uploaded content: Retained while account is active
- Purchased content: Available for download for 10 days after purchase
- Deleted content: Permanently removed within 30 days

### 9.2 User Data Retention

- Account information: Retained while account is active
- Transaction records: 7 years (legal requirement)
- Usage logs: 90 days
- Support communications: 2 years

### 9.3 Backup and Recovery

- Regular backups performed for disaster recovery
- Backups retained for 30 days
- Deleted data removed from backups within 60 days

---

## 10. Contact Information

### General Support
- Email: support@aimediatank.com
- Website: https://www.aimediatank.com

### Legal Inquiries
- Email: support@aimediatank.com
- Subject Line: "Legal Inquiry"

### DMCA/Copyright Issues
- Email: support@aimediatank.com
- Subject Line: "DMCA Notice"

### Privacy Concerns
- Email: support@aimediatank.com
- Subject Line: "Privacy Request"

---

## Disclaimer

THE PLATFORM IS PROVIDED "AS IS" WITHOUT WARRANTIES OF ANY KIND. WE DO NOT GUARANTEE UNINTERRUPTED ACCESS OR THAT THE PLATFORM WILL BE ERROR-FREE. TO THE MAXIMUM EXTENT PERMITTED BY LAW, AIMEDIATANK SHALL NOT BE LIABLE FOR ANY INDIRECT, INCIDENTAL, SPECIAL, OR CONSEQUENTIAL DAMAGES.

---

## Governing Law

These terms shall be governed by and construed in accordance with the laws of the United States, without regard to conflict of law principles.

---

## Severability

If any provision of these terms is found to be unenforceable, the remaining provisions will continue in full force and effect.

---

**© 2024 AiMediaTank. All Rights Reserved.**

---

*This Policy Book is a draft document and should be reviewed by legal counsel before official publication.*

